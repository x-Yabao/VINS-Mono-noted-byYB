...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const std_msgs::Header &header)
{
    ...
    //addFeatureCheckParallax()添加之前检测到的特征点到feature容器list中，计算每一个点跟踪的次数，以及它的视差并通过检测两帧之间的视差决定是否作为关键帧。
    ...
    if (f_manager.addFeatureCheckParallax(frame_count, image, td))
        // 如果上一帧（倒数第二帧）是关键帧，则滑窗中最老的帧就要被移出滑窗
    ...
    // Step 2： 外参初始化
    // 判断是否需要进行外参标定
    // VINS外参标定指的是对相机坐标系到IMU坐标系的旋转矩阵进行在线标定与优化
...
bool Estimator::initialStructure()
{
    ...
    // Step 2 global sfm
    // 做一个纯视觉slam
    Quaterniond Q[frame_count + 1];   //frame_count为10
    ...
    // step2 只是针对KF进行sfm，初始化需要all_image_frame中的所有元素，因此下面通过KF来求解其他的非KF的位姿
    map<double, ImageFrame>::iterator frame_it;
 ...
 *        这样其他帧都对齐到这个枢纽帧（参考帧）上
 *        得到T_l_last
 * @param[in] relative_R 最后一帧（当前帧）到第l帧（参考帧）的relative_R
 * @param[in] relative_T 最后一帧（当前帧）到第l帧（参考帧）的relative_T
 * @param[in] l 枢纽帧的索引
 * @return true 
 * @return false 
 */
...
// VIO非线性优化求解里程计,会调用 optimization()
...
/**
 * @brief 基于滑动窗口的紧耦合的非线性优化，残差项的构造和求解
 * 
 */
void Estimator::optimization()
{
    ...
    ceres::LossFunction *loss_function;   // 引入鲁棒核函数
    ...
    // 视觉重投影的约束
    // 遍历每一个特征点
    // IMU的残差是相邻两帧，但是视觉不是的,它加入的2帧，这两帧是观测到同一特征的最近两帧。
    for (auto &it_per_id : f_manager.feature)
    {
        it_per_id.used_num = it_per_id.feature_per_frame.size();
        // 进行特征点有效性的检查
...
void Estimator::slideWindowOld()
{
    ...
    if (shift_depth)    // 如果初始化过了
    {
        ...
        R0 = back_R0 * ric[0];  // 被移除的相机的姿态, 滑窗原先的最老帧(被merge掉)的旋转(c->w)
        R1 = Rs[0] * ric[0];    // 当前最老的相机姿态, 滑窗原先第二老的帧(现在是最老帧)的旋转(c->w)
