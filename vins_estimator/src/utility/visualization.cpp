...
nav_msgs::Path path, relo_path;   //保存路径
...
void registerPub(ros::NodeHandle &n)
{
    ...
    pub_key_poses = n.advertise<visualization_msgs::Marker>("key_poses", 1000);     //滑动窗口里的帧
...
void pubKeyframe(const Estimator &estimator)
{
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR && estimator.marginalization_flag == 0)
    {
        ...
        pub_keyframe_pose.publish(odometry);  // 发布关键帧位姿
        ...
        pub_keyframe_point.publish(point_cloud);  //发布关键帧能看到的点（三角化成功的）
    }
}
