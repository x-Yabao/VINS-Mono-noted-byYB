...
/**
 * @brief 添加之前检测到的特征点到feature容器list中，
 * 计算每一个点跟踪的次数，
...
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double td)
{
    ...
    {
        // 计算的实际上是frame_count-1,也就是前一帧（倒数第二帧）是否为关键帧（倒数第二帧，即MARGIN_SECOND_NEW的原因）
        // 因此起始帧至少得是frame_count - 2,同时至少覆盖到frame_count - 1帧
        if (it_per_id.start_frame <= frame_count - 2 &&
            it_per_id.start_frame + int(it_per_id.feature_per_frame.size()) - 1 >= frame_count - 1)
        {
            //计算特征点在倒数第二帧和倒数第三帧之间的视差
...
void FeatureManager::triangulate(Vector3d Ps[], Vector3d tic[], Matrix3d ric[])
{
    ...
    for (auto &it_per_id : feature)
    {
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame)
        {
            ...
            // T_w_cj（这里的w是指枢纽坐标系） -> T_c0_cj（c0表示第一个看到此特征点的帧）
