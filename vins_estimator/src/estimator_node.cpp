...
queue<sensor_msgs::ImuConstPtr> imu_buf;   //保持imu数据
...
/**
 * @brief 从IMU测量值imu_msg和上一个PVQ递推得到当前PVQ
 * 
 * @param[in] imu_msg 
 */
...
void process()
{
    ...
    {
        // measurements的数据格式可以表示为：(IMUs, img_msg)s s表示容器（vector）
        std::vector<std::pair<std::vector<sensor_msgs::ImuConstPtr>, sensor_msgs::PointCloudConstPtr>> measurements;
        ...
        for (auto &measurement : measurements)
        {
            ...
            // 2.2、回环相关部分，在relo_buf中取出最后一个重定位帧，拿出其中的信息并执行setReloFrame()
            ...
            for (unsigned int i = 0; i < img_msg->points.size(); i++)
            {
                ...
                int camera_id = v % NUM_OF_CAM;      //camera0
            ...
            ///2.5、向RVIZ发布里程计信息、关键位姿、相机位姿、点云和TF关系
...
int main(int argc, char **argv)
{
    ...
    // 3、注册一些publisher，发布用于RVIZ显示的Topic
    registerPub(n);
