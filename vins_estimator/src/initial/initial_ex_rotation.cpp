...
{
    frame_count++;
    // 根据特征关联求解两个连续帧相机的旋转R12，当前帧相对于上一帧的姿态
...
// solveRelativeR(corres)根据对极几何计算相邻帧间相机坐标系的旋转矩阵，
// 这里的corres传入的是当前帧和其之前一帧的对应特征点对的归一化坐标
Matrix3d InitialEXRotation::solveRelativeR(const vector<pair<Vector3d, Vector3d>> &corres)
{
    if (corres.size() >= 9)
    {
        // 1.将corres中对应点的二维坐标分别存入ll与rr中
        ...
        //4. 采用三角化对每组Rt解进行验证，选择是正深度的Rt解
        double ratio1 = max(testTriangulation(ll, rr, R1, t1), testTriangulation(ll, rr, R1, t2));
        double ratio2 = max(testTriangulation(ll, rr, R2, t1), testTriangulation(ll, rr, R2, t2));
        cv::Mat_<double> ans_R_cv = ratio1 > ratio2 ? R1 : R2;

        // 解出来的是R21
        //5. 这里的R是上一帧到当前帧的变换矩阵，对其求转置为当前帧相对于上一帧的姿态
