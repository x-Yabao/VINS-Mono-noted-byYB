...
class IntegrationBase
{
  public:
    ...
    // 用_acc_0初始化acc_0和linearized_acc(分别是i时刻和bk时刻)(i时刻是IMU，bk时刻是图像帧)
    // 用_gyr_0初始化gyr_0和linearized_gyr(分别是i时刻和bk时刻)
    // 用15x15的单位矩阵初始化J
    // 用15x15的0矩阵初始化协方差传递矩阵
    // 用yaml文件里读取的IMU噪声参数初始化18x18的噪声矩阵
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        Vector3d un_gyr = 0.5 * (_gyr_0 + _gyr_1) - linearized_bg;   //角速度均值
        result_delta_q = delta_q * Quaterniond(1, un_gyr(0) * _dt / 2, un_gyr(1) * _dt / 2, un_gyr(2) * _dt / 2);  //计算位姿的预积分
    ...
    // 针对的是bk和bk+1内部的i时刻到i+1时刻的PVQ传播和误差传递。
    ...
    double dt;
    Eigen::Vector3d acc_0, gyr_0;  // 上一时刻（i）加速度计和陀螺仪的值
    Eigen::Vector3d acc_1, gyr_1;  // 当前时刻（i+1）加速度计和陀螺仪的值

    const Eigen::Vector3d linearized_acc, linearized_gyr;  //bk时刻加速度和角速度
    Eigen::Vector3d linearized_ba, linearized_bg;   //加速度计和陀螺仪偏置
    ...
    Eigen::Quaterniond delta_q;   //位姿预积分
