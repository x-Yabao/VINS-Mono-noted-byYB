...
void PoseGraph::registerPub(ros::NodeHandle &n)
{
    ...
    pub_pose_graph = n.advertise<visualization_msgs::MarkerArray>("pose_graph", 1000);   //连接两个回环帧的那根红线
    for (int i = 1; i < 10; i++)
...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    Vector3d vio_P_cur;
    Matrix3d vio_R_cur;
    // Step 1 如果sequence_cnt != cur_kf->sequence，则新建一个新的图像序列
    // 一开始先判断最新帧的序列号和poseGraph里现有的序列号是否相同，如果是不同的序列，
    // 当前帧是全新的序列，那么世界坐标系矫正矩阵和漂移矫正矩阵都重置
    ...
    // Step 2 把当前帧的里程计进行世界坐标系的矫正（sequence矫正），再还给它
	...
	if (loop_index != -1)   
	{
        ...
        if (cur_kf->findConnection(old_kf))             // 如果确定两者回环
        {
            ...
            // 获取当前帧与回环帧的相对位姿 relative_q、relative_t
            Vector3d relative_t;
    ...
    Vector3d P;
    Matrix3d R;
    // Step 5 获取VIO当前帧的位姿P、R，根据偏移量计算得到实际位姿,并进行位姿更新
    cur_kf->getVioPose(P, R);
    P = r_drift * P + t_drift;  // 根据全局优化后进行位姿调整
    R = r_drift * R;
    cur_kf->updatePose(P, R);
    // 下面是可视化部分
    // Step 6 发布path[sequence_cnt]
    Quaterniond Q{R};
    ...
    // Step 7 保存闭环轨迹到VINS_RESULT_PATH
    if (SAVE_LOOP_PATH)
    {
    ...
    // Step 8 绘制可视化轨迹中帧间的连线，发布topic：pub_pg_path、pub_path、pub_base_path
...
/**
 * 该函数用于检测当前帧与先前帧是否可能存在回环，若存在则返回回环候选帧的索引。
...
int PoseGraph::detectLoop(KeyFrame* keyframe, int frame_index)
{
    ...
    // Step 1 查询字典数据库，得到与每一帧的相似度评分ret
    QueryResults ret;
    TicToc t_query;
    ...
    TicToc t_add;
    // Step 2 添加当前关键帧到字典数据库中，便于后续帧的查询
    ...
    // a good _ with its nerghbour
    // Step 3 通过相似度评分判断是否存在回环候选帧
