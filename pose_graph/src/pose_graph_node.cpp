...
queue<sensor_msgs::PointCloudConstPtr> point_buf;   //关键帧的点云
queue<nav_msgs::Odometry::ConstPtr> pose_buf;       //关键帧的pose
queue<Eigen::Vector3d> odometry_buf;
std::mutex m_buf;
std::mutex m_process;
int frame_index  = 0;       // 关键帧的索引
int sequence = 1;           // 关键帧所在序列号，初始化为1
PoseGraph posegraph;
int skip_first_cnt = 0;
int SKIP_CNT;
int skip_cnt = 0;
bool load_flag = 0;         // 是否已经加载之前的位姿图
bool start_flag = 0;
double SKIP_DIS = 0;
...
int LOOP_CLOSURE;           //是否进行回环
int FAST_RELOCALIZATION;

camodocal::CameraPtr m_camera;
Eigen::Vector3d tic;        //外参
Eigen::Matrix3d qic;
...
void process()
{
    ...
    while (true)
    {
        ...
        // Step 1 得到具有相同时间戳的 pose_msg、image_msg、point_msg
        m_buf.lock();
        ...
        // Step 2 选取构建pose_graph中用到的关键帧
        // 这里用到的策略是先剔除最开始的SKIP_FIRST_CNT帧，
        // 然后每隔SKIP_CNT，将将距上一关键帧距离（平移向量的模）超过SKIP_DIS的图像创建为关键帧 
        if (pose_msg != NULL)   // 判断一下是否有效
        {
            ...
            if((T - last_t).norm() > SKIP_DIS)          // 要求KF相隔必要的平移距离
            {
                ...
                m_process.lock();
                start_flag = 1;
                // Step 4 在posegraph中添加关键帧，将flag_detect_loop=1即设置回环检测
...
int main(int argc, char **argv)
{
    ...
    pub_match_img = n.advertise<sensor_msgs::Image>("match_image", 1000);   //回环匹配成功的图片
    ...
    measurement_process = std::thread(process);         //pose graph主线程
    keyboard_command_process = std::thread(command);    //键盘操作的线程
